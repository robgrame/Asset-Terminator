...
# DecommissionStatus — HTTP GET v1/decommission/{requestId}
# Parity with AssetTerminator.Api DecommissionQueryFunction.GetStatus.
# Returns the current aggregate status for a request (used by ServiceNow polling).
...
try {
    ...
    Write-AtLog -Level 'Error' -Message "Status lookup failed: $($_.Exception.Message)" -Properties @{ requestId = $requestId }
...
Write-HttpJson -StatusCode 200 -Body (ConvertTo-StatusResponse -Record $record)
