...
// PowerShell parallel of infra/main.bicep. Provisions the same topology (monitoring, identities,
// SQL, storage+WORM, Service Bus, Key Vault, RBAC) but hosts the API and orchestrator on the
// PowerShell 7.4 Flex Consumption runtime. The runtime-agnostic modules are copied verbatim from
// the .NET infra; only functionapp.bicep differs.
...
module apiFunction './modules/functionapp.bicep' = {
  ...
  params: {
    ...
    auditStorageAccountName: storage.outputs.auditStorageName
    auditContainerName: storage.outputs.auditContainerName
...
output auditStorageAccountName string = storage.outputs.auditStorageName
